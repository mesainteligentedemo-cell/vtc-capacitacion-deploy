class ModuleController {
  ...
  async loadModulo(moduloId) {
    ...
    try {
      const response = await fetch(`${window.API_BASE}/api/timeline/${moduloId}`);
  ...
  async submitQuiz() {
    ...
    try {
      const response = await fetch(`${window.API_BASE}/api/quiz/submit`, {
        method: 'POST',
  ...
  async saveProgress(status = 'in_progress') {
    ...
    try {
      const response = await fetch(`${window.API_BASE}/api/usuarios/${this.usuarioId}/save-progress`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          modulo: this.currentModulo,
